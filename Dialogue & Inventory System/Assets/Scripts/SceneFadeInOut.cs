...
{
	// -- Singleton Structure
	protected static SceneFadeInOut mInstance;
	public static SceneFadeInOut Instance
	{
		get
		{
			if (mInstance == null)
			{
				...
				mInstance = tempObj.AddComponent<SceneFadeInOut>();
				Destroy(tempObj);
			}
			return mInstance;
		}
	...
	void DoFade(bool Mode)
	{
		...
		if (Mode)
		{
			if (FadeSprite.GetComponentInChildren<Image>().color.a > 0.0f)
			{
				...
				AudioListener.volume = (1.0f-f_Alpha); //ADDED: FADES OUT GLOBAL VOLUME
			}
	...
	public void StartFadeMenu(string SceneName)
	{
		bool Mode = false;
		// -- Do not constantly reset fade
		//if (b_DoFade) return;
